...
const S = {
  ...
  drag: null,
  pan: null,
  cam: { x: 0.5, y: 0.15, z: 400 },
  snap: { on: false, inc: 0.025, pts: false },
  ref: {
    visible: true,
    img: null,
    x: 0.5,
...
function w2s(wx, wy) {
  return [
    ...
    CH() / 2 - (wy - S.cam.y) * S.cam.z,
  ];
}
function s2w(sx, sy) {
  return [
    S.cam.x + (sx - CW() / 2) / S.cam.z,
...
function snapXY(wx, wy, excludePts) {
  ...
  if (S.snap.on) {
    const i = S.snap.inc;
    rx = Math.round(wx / i) * i;
    ry = Math.round(wy / i) * i;
  }
  if (S.snap.pts) {
    ...
    for (const ly of S.layers) {
      ...
      (ly.pts || []).forEach((p, i) => {
        if (
          excludePts &&
          excludePts.some((s) => s.lid === ly.id && s.idx === i)
        )
          return;
...
// UNDO
function pushUndo() {
  U.stack.push({ layers: JSON.stringify(S.layers), activeId: S.activeId });
  if (U.stack.length > U.MAX) U.stack.shift();
  U.redo = [];
}
function applyUndoState(st) {
  S.layers = JSON.parse(st.layers);
  S.activeId = st.activeId;
  if (!getLayer(S.activeId)) S.activeId = "outline";
  S.selSet = [];
}
function undo() {
  if (!U.stack.length) return;
  U.redo.push({ layers: JSON.stringify(S.layers), activeId: S.activeId });
  applyUndoState(U.stack.pop());
  draw();
}
function redo() {
  if (!U.redo.length) return;
  U.stack.push({ layers: JSON.stringify(S.layers), activeId: S.activeId });
  applyUndoState(U.redo.pop());
  draw();
}
...
function copyPts() {
  ...
  const ly = activeLy();
  if (!ly) return;
  const sorted = [...S.selSet]
    .filter((s) => s.lid === ly.id)
    .sort((a, b) => a.idx - b.idx);
  if (!sorted.length) return;
  S.clipboard = {
    layerType: ly.type,
    pts: JSON.parse(JSON.stringify(sorted.map((s) => ly.pts[s.idx]))),
  };
}
function pastePts() {
  ...
  const ly = activeLy();
  if (!ly) return;
  if (ly.type !== S.clipboard.layerType) {
    alert("Cannot paste: incompatible layer type.");
    return;
  }
  if (
    ly.pts.length &&
    !confirm(
      "Pasting into a non-empty layer may disrupt point order. Continue?",
    )
  )
    return;
  pushUndo();
...
function crCtrl(p0, p1, p2, p3, a = 0.5) {
  const tx1 = p1.tangentOut
    ...
    : p1.tangent
      ? p1.tangent.x
      : a * (p2.x - p0.x);
  const ty1 = p1.tangentOut
    ? p1.tangentOut.y
    : p1.tangent
      ? p1.tangent.y
      : a * (p2.y - p0.y);
  const tx2 = p2.tangentIn
    ? p2.tangentIn.x
    : p2.tangent
      ? p2.tangent.x
      : a * (p3.x - p1.x);
  const ty2 = p2.tangentIn
    ? p2.tangentIn.y
    : p2.tangent
      ? p2.tangent.y
      : a * (p3.y - p1.y);
  return {
    c1: { x: p1.x + tx1 / 3, y: p1.y + ty1 / 3 },
    c2: { x: p2.x - tx2 / 3, y: p2.y - ty2 / 3 },
  };
}
...
function tracePath(ctx2, pts, closed, xf) {
  ...
  for (let i = 0; i < segs; i++) {
    ...
    if (p1.smooth && p2.smooth) {
      ...
      ctx2.bezierCurveTo(...tf(c1.x, c1.y), ...tf(c2.x, c2.y), ex, ey);
...
function bakeSpline(pts, closed, res) {
  ...
  for (let i = 0; i < (closed ? n : n - 1); i++) {
    ...
    if (p1.smooth && p2.smooth) {
      const { c1, c2 } = crCtrl(p0, p1, p2, p3);
      for (let j = 1; j < res; j++) {
        const { x, y } = bezPt(p1, c1, c2, p2, j / res);
        out.push([r4(x), r4(y)]);
      }
    }
  }
  ...
  for (let i = 0; i < (ly.closed ? n : n - 1); i++) {
    ...
    if (p1.smooth && p2.smooth) {
      ...
      for (let j = 0; j <= SAMPLES; j++) {
        const t = j / SAMPLES,
          pt = bezPt(p1, c1, c2, p2, t);
        const [bsx, bsy] = w2s(pt.x, pt.y),
          d = Math.hypot(sx - bsx, sy - bsy);
        if (d < bestD) {
          bestD = d;
          best = { idx: i + 1, t };
        }
      ...
      const t = Math.max(
        0,
        Math.min(1, ((sx - x1) * dx + (sy - y1) * dy) / l2),
      );
...
function insertOnSegment(ly, hit) {
  ...
  if (p1.smooth && p2.smooth) {
    ...
    newPt = {
      x: r4(p1.x + (p2.x - p1.x) * t),
      y: r4(p1.y + (p2.y - p1.y) * t),
      smooth: false,
    };
  }
...
// LAYER HELPERS
const mkPoly = (pts, color, id, name) => ({
  id,
  name,
  type: "poly",
  closed: true,
...
const getPt = (s) => {
  const ly = getLayer(s.lid);
  return ly ? (ly.pts || [])[s.idx] || null : null;
};
const selLast = () => (S.selSet.length ? S.selSet[S.selSet.length - 1] : null);
const inSelSet = (s) =>
  S.selSet.some((t) => t.lid === s.lid && t.idx === s.idx);
...
function rotHandleSS(bb) {
  ...
  const topSy = Math.min(...cs.map((c) => c[1])),
    cx = cs.reduce((s, c) => s + c[0], 0) / 4;
...
function addFlag() {
  const fi = flagCount++,
    col = FLAG_COLS[fi % FLAG_COLS.length];
  const ins = S.layers.findIndex((l) => l.id === "inputs");
  pushUndo();
  S.layers.splice(
    ins >= 0 ? ins : S.layers.length,
    0,
    mkPoly([], col, `flag_${fi}`, `Flag ${fi}`),
  );
  S.activeId = `flag_${fi}`;
  S.selSet = [];
  draw();
...
function mirrorH() {
  const ly = activeLy();
  if (!ly) return;
  const pts = S.selSet.length
    ? S.selSet.map((s) => getPt(s)).filter(Boolean)
    : ly.pts || [];
  ...
  pts.forEach((p) => {
    p.x = r4(2 * cx - p.x);
    if (p.tangent) p.tangent.x = -p.tangent.x;
    if (p.tangentOut) p.tangentOut.x = -p.tangentOut.x;
    if (p.tangentIn) p.tangentIn.x = -p.tangentIn.x;
  });
  draw();
}
function alignH() {
...
function mirrorV() {
  ...
  pts.forEach((p) => {
    p.y = r4(2 * cy - p.y);
    if (p.tangent) p.tangent.y = -p.tangent.y;
    if (p.tangentOut) p.tangentOut.y = -p.tangentOut.y;
    if (p.tangentIn) p.tangentIn.y = -p.tangentIn.y;
  });
  draw();
}
...
function commitShape(tool, x0, y0, x1, y1) {
  ...
  if (tool === "rect") {
    pts = [
      { x: r4(mnX), y: r4(mnY), smooth: false },
      { x: r4(mxX), y: r4(mnY), smooth: false },
      { x: r4(mxX), y: r4(mxY), smooth: false },
      { x: r4(mnX), y: r4(mxY), smooth: false },
    ];
  } else if (tool === "tri") {
    pts = [
      { x: r4(cx), y: r4(mxY), smooth: false },
      { x: r4(mxX), y: r4(mnY), smooth: false },
      { x: r4(mnX), y: r4(mnY), smooth: false },
    ];
    ...
    pts = [
      {
        ...
        y: r4(cy - hh),
        smooth: true,
        tangent: { x: r4(3 * K * hw), y: 0 },
      },
      {
        x: r4(cx + hw),
        y: r4(cy),
        smooth: true,
        tangent: { x: 0, y: r4(3 * K * hh) },
      },
      {
        x: r4(cx),
        y: r4(cy + hh),
    ...
    if (hw >= hh) {
      ...
      pts = [
        ...
        { x: r4(cx + hw), y: r4(cy), smooth: true, tangent: { x: 0, y: -t } }, // rightmost,      tangent ↓
        {
          ...
          smooth: true,
          tangent: { x: -t, y: 0 },
        }, // bot-right join, tangent ←
        {
          x: r4(cx - hw + r),
          y: r4(cy - r),
          smooth: true,
          tangent: { x: -t, y: 0 },
        }, // bot-left join,  tangent ←
...
function draw() {
  ...
  const al = activeLy();
  for (const ly of S.layers) {
    if (!ly.visible || ly === al) continue;
    drawLayer(ly, false);
  }
  if (al) drawLayer(al, true);
  if (["rect", "tri", "ellipse", "capsule"].includes(S.drag?.type))
    drawShapePreview();
...
function toggleRefFlip(axis) {
  S.ref[axis] = !S.ref[axis];
  draw();
}

function toggleRefLayer() {
  S.ref.open = !S.ref.open;
  document.getElementById("ref-controls").style.display = S.ref.open
    ? "block"
    : "none";
  document.getElementById("ref-arrow").textContent = S.ref.open ? "▼" : "▶";
}

const REF_DEFAULTS = { x: 0.5, y: 0.15, scale: 1.0, rotation: 0, opacity: 0.1 };
function resetRefParam(e, param) {
  if (!e.ctrlKey || e.button !== 1) return;
  e.preventDefault();
  const val = REF_DEFAULTS[param];
  S.ref[param] = val;
  const fmt = (v) =>
    typeof v === "number" && param !== "rotation" ? v.toFixed(2) : String(v);
...
function drawGrid() {
  ...
  if (S.snap.on && S.cam.z * S.snap.inc >= 7) {
    const inc = S.snap.inc,
      [minWX, minWY] = s2w(0, CH()),
      [maxWX, maxWY] = s2w(CW(), 0);
    ctx.fillStyle = "rgba(255,255,255,0.2)";
    for (let x = Math.ceil(minWX / inc) * inc; x <= maxWX; x += inc)
      for (let y = Math.ceil(minWY / inc) * inc; y <= maxWY; y += inc) {
        const [sx, sy] = w2s(x, y);
        ctx.beginPath();
...
function drawLayer(ly, active) {
  ...
  if (ly.type === "poly") {
    if (ly.pts.length > 1) {
      ctx.beginPath();
      tracePath(ctx, ly.pts, ly.closed);
      ctx.strokeStyle = ly.color;
      ctx.lineWidth = active ? 2 : 1;
      ctx.stroke();
      if (active) {
        ctx.fillStyle = ly.color + "18";
        ctx.fill();
      }
    }
...
function drawPolyPoints(ly) {
  const sl = selLast();
  if (sl && sl.lid === ly.id && S.selSet.length === 1) {
    const p = ly.pts[sl.idx];
    if (p && p.smooth) drawHandles(ly.pts, sl.idx);
  }
  for (let i = 0; i < ly.pts.length; i++) {
    const p = ly.pts[i],
      [sx, sy] = w2s(p.x, p.y),
      isSel = inSelSet({ lid: ly.id, idx: i });
    ctx.beginPath();
    ctx.arc(sx, sy, isSel ? 7 : 5, 0, Math.PI * 2);
    if (isSel) {
      ctx.fillStyle = "#fff";
      ctx.strokeStyle = "#111";
      ctx.lineWidth = 1.5;
    } else {
      ctx.fillStyle = p.smooth ? "#4a9de0" : "#d06060";
      ctx.strokeStyle = "#111";
      ctx.lineWidth = 1;
    }
    ctx.fill();
...
function drawHandles(pts, idx) {
  const p = pts[idx],
    autoT = getTangent(pts, idx);
  const tOut = p.tangentOut || p.tangent || autoT;
  const tIn = p.tangentIn || p.tangent || autoT;
  const hasOut = !!(p.tangentOut || p.tangent),
    hasIn = !!(p.tangentIn || p.tangent);
  const hOut = { x: p.x + tOut.x / 3, y: p.y + tOut.y / 3 },
    hIn = { x: p.x - tIn.x / 3, y: p.y - tIn.y / 3 };
  const [hox, hoy] = w2s(hOut.x, hOut.y),
    [hix, hiy] = w2s(hIn.x, hIn.y),
    [px, py] = w2s(p.x, p.y);
  ctx.save();
  ctx.lineWidth = 1;
  ctx.setLineDash([3, 3]);
  ctx.strokeStyle = "rgba(255,200,100,.4)";
  ctx.beginPath();
  ctx.moveTo(px, py);
  ctx.lineTo(hox, hoy);
  ctx.stroke();
  ctx.strokeStyle = "rgba(255,130,60,.4)";
  ctx.beginPath();
  ctx.moveTo(px, py);
  ctx.lineTo(hix, hiy);
  ctx.stroke();
  ...
  ctx.fillStyle = hasOut ? "#ffcc44" : "#888";
  ctx.strokeStyle = "#111";
  ctx.lineWidth = 1;
  ctx.fill();
  ctx.stroke();
  ...
  ctx.fillStyle = hasIn ? "#ff8844" : "#888";
  ctx.strokeStyle = "#111";
  ctx.lineWidth = 1;
  ctx.fill();
  ctx.stroke();
...
function drawPorts(ly, active) {
  for (let i = 0; i < ly.pts.length; i++) {
    ...
    ctx.lineTo(sx - sz, sy);
    ctx.closePath();
    ctx.fillStyle = isSel ? "#fff" : ly.color;
    ctx.strokeStyle = "#111";
    ctx.lineWidth = 1;
    ctx.fill();
    ctx.stroke();
...
function drawIconBox(ly, active) {
  ...
  const p0 = ly.pts[0],
    p1 = ly.pts[1];
  const mnX = Math.min(p0.x, p1.x),
    mnY = Math.min(p0.y, p1.y),
    mxX = Math.max(p0.x, p1.x),
    mxY = Math.max(p0.y, p1.y);
  const corners = [
    [mnX, mnY],
    [mxX, mnY],
    [mxX, mxY],
...
function drawTransformGizmo() {
  ...
  if (!bb) return;
  ctx.save();
  const [x0, y0] = w2s(bb.minX, bb.minY),
    [x1, y1] = w2s(bb.maxX, bb.maxY);
  ...
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.moveTo(ccx - 3, ccy);
  ctx.lineTo(ccx + 3, ccy);
...
function drawShapePreview() {
  ...
  const tf = w2s;
  ctx.save();
  ctx.strokeStyle = "rgba(232,160,32,.85)";
  ctx.lineWidth = 1.5;
  ctx.setLineDash([4, 3]);
  ctx.beginPath();
  if (d.type === "rect") {
    const [sx0, sy0] = tf(mnX, mnY),
      [sx1, sy1] = tf(mxX, mxY);
    ctx.rect(
      Math.min(sx0, sx1),
      Math.min(sy0, sy1),
    ...
    ctx.moveTo(...tf(cx, mxY));
    ctx.lineTo(...tf(mxX, mnY));
    ctx.lineTo(...tf(mnX, mnY));
    ctx.closePath();
    ...
    if (hw >= hh) {
      const r = hh;
      const [rcx, rcy] = w2s(cx + hw - r, cy),
        [lcx, lcy] = w2s(cx - hw + r, cy);
      const sr = r * S.cam.z;
      ...
      const r = hw;
      const [tcx, tcy] = w2s(cx, cy + hh - r),
        [bcx, bcy] = w2s(cx, cy - hh + r);
      const sr = r * S.cam.z;
...
function drawMarquee() {
  const d = S.drag;
  if (!d || d.type !== "marquee") return;
  const [sx0, sy0] = w2s(d.startWx, d.startWy),
    [sx1, sy1] = w2s(d.curWx, d.curWy);
  ctx.save();
  ctx.fillStyle = "rgba(100,160,255,.07)";
  ctx.strokeStyle = "rgba(100,160,255,.6)";
  ctx.lineWidth = 1;
  ctx.setLineDash([4, 3]);
  ctx.fillRect(
    Math.min(sx0, sx1),
...
function updatePreview() {
  ...
  pv.width = pv.offsetWidth * dpr;
  pv.height = (pv.offsetHeight || 84) * dpr;
  pctx.clearRect(0, 0, pv.width, pv.height);
  pctx.fillStyle = "#2a2a2a";
  pctx.fillRect(0, 0, pv.width, pv.height);
  const outline = getLayer("outline");
  if (!outline || outline.pts.length < 2) return;
  let mnX = 0,
    mnY = 0,
    mxX = 1,
    mxY = 0.3;
  ...
  const wR = mxX - mnX,
    ...
    PAD = 12 * dpr;
  const scale =
    Math.min((pv.width - PAD * 2) / wR, (pv.height - PAD * 2) / hR) * 0.88;
  const ox = PAD + (pv.width - PAD * 2 - wR * scale) / 2 - mnX * scale,
    oy = PAD + (pv.height - PAD * 2 - hR * scale) / 2 + mxY * scale;
  const tf = (wx, wy) => [ox + wx * scale, oy - wy * scale],
    res = bakeRes();
  for (const ly of S.layers) {
    if (!ly.id.startsWith("flag_") || !ly.visible || ly.pts.length < 2)
      continue;
    pctx.beginPath();
    pctx.moveTo(...tf(ly.pts[0].x, ly.pts[0].y));
    for (let i = 1; i < ly.pts.length; i++)
      pctx.lineTo(...tf(ly.pts[i].x, ly.pts[i].y));
    pctx.closePath();
    pctx.fillStyle = "rgba(80,130,160,.28)";
    pctx.fill();
  }
  const baked = bakeSpline(outline.pts, true, res);
  if (baked.length) {
    pctx.beginPath();
    pctx.moveTo(...tf(...baked[0]));
    for (let i = 1; i < baked.length; i++) pctx.lineTo(...tf(...baked[i]));
    pctx.closePath();
    pctx.fillStyle = "rgba(145,195,220,.65)";
    pctx.strokeStyle = "#e8a020";
  ...
  for (const ly of S.layers) {
    ...
    for (const p of ly.pts) {
      pctx.beginPath();
      pctx.arc(...tf(p.x, p.y), 4 * dpr, 0, Math.PI * 2);
      pctx.fillStyle = "#777";
      pctx.fill();
    }
  }
...
function buildJSON() {
  ...
  let fi = 0;
  for (const ly of S.layers) {
    if (!ly.id.startsWith("flag_")) continue;
    flags[fi++] = { outline: bakeSpline(ly.pts, true, res) };
  }
  const inp = getLayer("inputs"),
    out = getLayer("outputs"),
    ico = getLayer("icon");
  return {
    name,
    flags,
    outline: outline ? bakeSpline(outline.pts, true, res) : [],
...
function dlJSON() {
  const name = document.getElementById("name-in").value || "myshape";
...
function importFile(e) {
  ...
  if (
    !confirm(
      'Replace current shape with "' + file.name + '"?\nThis cannot be undone.',
    )
  )
    return;
...
function loadJSON(json) {
  ...
  if (json.outline)
    S.layers.push(
      mkPoly(
        json.outline.map(([x, y]) => ({ x, y, smooth: false })),
        "#e8a020",
        "outline",
        "Outline",
      ),
  ...
  if (json.flags) {
    const keys = Object.keys(json.flags)
      .map(Number)
      .sort((a, b) => a - b);
    for (const k of keys) {
      S.layers.push(
        mkPoly(
          json.flags[k].outline.map(([x, y]) => ({ x, y, smooth: false })),
          FLAG_COLS[k % FLAG_COLS.length],
          `flag_${k}`,
          `Flag ${k}`,
        ),
      );
      flagCount = Math.max(flagCount, k + 1);
    }
  }
  const pp = (id, name, col, arr, defA) =>
    S.layers.push({
      id,
      name,
      type: "ports",
      visible: true,
      color: col,
      pts: (arr || []).map(([x, y, a]) => ({ x, y, angle: a ?? defA })),
    });
  pp("inputs", "Inputs", "#5db55d", json.inputs, 90);
  pp("outputs", "Outputs", "#d05555", json.outputs, 270);
  S.layers.push({
    id: "icon",
    name: "Icon",
    type: "icon",
...
function buildLayerUI() {
  ...
  [...list.children].forEach((el) => {
    if (el.id !== "ref-layer") el.remove();
  });
  for (const ly of S.layers) {
    const div = document.createElement("div");
    div.className = "ly" + (ly.id === S.activeId ? " active" : "");
    div.onclick = (e) => {
      if (
        e.target.classList.contains("ly-del") ||
        e.target.classList.contains("ly-vis")
      )
        return;
      S.activeId = ly.id;
      S.selSet = [];
    ...
    vis.className = "ly-vis" + (ly.visible ? " on" : "");
    vis.title = "Toggle display only";
    vis.onclick = (e) => {
      e.stopPropagation();
    ...
    dot.style.background = ly.color;
    const nm = Object.assign(document.createElement("span"), {
      className: "ly-name",
      textContent: ly.name,
    });
    ...
    if (ly.id.startsWith("flag_")) {
      ...
      del.onclick = (e) => {
        e.stopPropagation();
        pushUndo();
        S.layers = S.layers.filter((l) => l.id !== ly.id);
        if (S.activeId === ly.id) S.activeId = "outline";
        draw();
      };
      div.append(del);
    }
...
function setSmoothProp(smooth) {
  if (!S.selSet.length) return;
  pushUndo();
  for (const sl of S.selSet) {
    const ly = getLayer(sl.lid);
    if (!ly || ly.type !== "poly") continue;
    const p = ly.pts[sl.idx];
    if (!p) continue;
    p.smooth = smooth;
    if (!smooth) {
...
const smoothSel = () => setSmoothProp(true),
  cornerSel = () => setSmoothProp(false);
...
function togglePanel(which) {
  ...
  const anyOpen =
    previewSec.style.display === "flex" || jsonSec.style.display === "flex";
...
function setTool(t) {
  ...
  S.selSet = [];
  S.drag = null;
  ["select", "add", "delete", "rect", "tri", "ellipse", "capsule"].forEach(
    (id) => {
      const btn = document.getElementById("tb-" + id);
...
function toggleSnap() {
  S.snap.on = !S.snap.on;
  document.getElementById("tb-snap").classList.toggle("snap-on", S.snap.on);
  draw();
}
function toggleSnapPts() {
  S.snap.pts = !S.snap.pts;
  document.getElementById("tb-ptsnap").classList.toggle("snap-on", S.snap.pts);
  draw();
}
...
function drawPresetThumb(canvas, data) {
  ...
  const wR = Math.max(mxX - mnX, 0.01),
    hR = Math.max(mxY - mnY, 0.01),
    PAD = 5;
  const scale =
    Math.min((canvas.width - PAD * 2) / wR, (canvas.height - PAD * 2) / hR) *
    0.88;
  const ox = PAD + (canvas.width - PAD * 2 - wR * scale) / 2 - mnX * scale,
    oy = PAD + (canvas.height - PAD * 2 - hR * scale) / 2 + mxY * scale;
  const tf = (x, y) => [ox + x * scale, oy - y * scale];
  c.fillStyle = "#1e1e1e";
  c.fillRect(0, 0, canvas.width, canvas.height);
  if (data.flags) {
    for (const k of Object.keys(data.flags)) {
      const fl = data.flags[k].outline;
      if (!fl || fl.length < 2) continue;
      c.beginPath();
      c.moveTo(...tf(...fl[0]));
      for (let i = 1; i < fl.length; i++) c.lineTo(...tf(...fl[i]));
      c.closePath();
      c.fillStyle = "rgba(60,110,140,.35)";
      c.fill();
    }
  }
  c.beginPath();
  c.moveTo(...tf(...outline[0]));
  for (let i = 1; i < outline.length; i++) c.lineTo(...tf(...outline[i]));
  c.closePath();
  c.fillStyle = "rgba(145,195,220,.6)";
...
function openPresets() {
  ...
  for (const [name, data] of Object.entries(PRESETS)) {
    ...
    cell.onclick = () => {
      if (
        !confirm(
          'Replace current shape with "' + name + '"?\nThis cannot be undone.',
        )
      )
        return;
      pushUndo();
      loadJSON(JSON.parse(JSON.stringify(data)));
      closePresets();
    };
...
function hitTransformHandle(sx, sy) {
  ...
  const rh = rotHandleSS(bb);
  if (Math.hypot(sx - rh.tip[0], sy - rh.tip[1]) < 12)
    return { type: "rotate" };
  // Centroid translate handle
  const [csx, csy] = w2s((bb.minX + bb.maxX) / 2, (bb.minY + bb.maxY) / 2);
  if (Math.hypot(sx - csx, sy - csy) < 10) return { type: "translate" };
  for (const h of gizmoHandles(bb)) {
    const [hsx, hsy] = w2s(h.wx, h.wy);
    if (Math.hypot(sx - hsx, sy - hsy) < HANDLE_HIT_R) return h;
  }
  return null;
}
...
function hitAngleTip(sx, sy) {
  const ly = activeLy();
  if (!ly || ly.type !== "ports") return null;
  for (let i = 0; i < ly.pts.length; i++) {
    const p = ly.pts[i],
      [psx, psy] = w2s(p.x, p.y);
    const rad = (p.angle * Math.PI) / 180;
    if (
      Math.hypot(
        sx - (psx + Math.cos(rad) * 22),
        sy - (psy + Math.sin(rad) * 22),
      ) < 9
    )
      return { lid: ly.id, idx: i };
  }
  return null;
}
...
function hitIconCorner(sx, sy) {
  ...
  const ly = activeLy();
  if (!ly || ly.type !== "icon" || ly.pts.length < 2) return null;
  const p0 = ly.pts[0],
    p1 = ly.pts[1];
  const mnX = Math.min(p0.x, p1.x),
    mnY = Math.min(p0.y, p1.y),
  ...
  for (let i = 0; i < 4; i++) {
    const [wx, wy] = corners[i],
      [csx, csy] = w2s(wx, wy);
    if (Math.hypot(sx - csx, sy - csy) < 9) return i;
  }
  return null;
}
...
function hitPoint(sx, sy) {
  ...
  if (S.selSet.length === 1 && ly.type === "poly") {
    ...
    if (p && p.smooth) {
      ...
      const tOut = p.tangentOut || p.tangent || autoT,
        tIn = p.tangentIn || p.tangent || autoT;
      const hOut = { x: p.x + tOut.x / 3, y: p.y + tOut.y / 3 },
        hIn = { x: p.x - tIn.x / 3, y: p.y - tIn.y / 3 };
      for (const [side, h] of [
        ["out", hOut],
        ...
        const [hsx, hsy] = w2s(h.x, h.y);
        if (Math.hypot(sx - hsx, sy - hsy) < HANDLE_HIT_R)
          return { lid: ly.id, idx: sl.idx, handle: side };
      }
    }
  }
  const pts = ly.pts || [];
  for (let i = 0; i < pts.length; i++) {
    const [px, py] = w2s(pts[i].x, pts[i].y);
    if (Math.hypot(sx - px, sy - py) < HIT_R) return { lid: ly.id, idx: i };
  }
  return null;
}
...
function addPointAt(wx, wy, sx, sy) {
  ...
  if (ly.type === "poly") {
    const [snx, sny] = snapXY(wx, wy),
      hit = hitSegment(sx, sy);
    if (hit && ly.pts.length >= 2) {
      insertOnSegment(ly, hit);
    } else {
      ly.pts.push({ x: r4(snx), y: r4(sny), smooth: true });
      S.selSet = [{ lid: ly.id, idx: ly.pts.length - 1 }];
    }
  } else if (ly.type === "ports") {
    ly.pts.push({ x: r4(wx), y: r4(wy), angle: ly.id === "inputs" ? 90 : 270 });
    S.selSet = [{ lid: ly.id, idx: ly.pts.length - 1 }];
  }
  draw();
}
...
cv.addEventListener("mousedown", (e) => {
  ...
  if (e.button === 1) {
    S.pan = { mx: e.clientX, my: e.clientY, cx: S.cam.x, cy: S.cam.y };
    e.preventDefault();
    return;
  }
  ...
  if (S.tool === "delete") {
    ...
    if (!hit) return;
    const l = getLayer(hit.lid);
    if (!l) return;
    pushUndo();
    (l.pts || []).splice(hit.idx, 1);
    S.selSet = [];
    draw();
    return;
  }
  ...
  if (th) {
    ...
    } else if (th.type === "translate") {
      S.drag = { type: "translate", startWx: wx, startWy: wy, origPts };
    } else {
      const cx = (bb.minX + bb.maxX) / 2,
        cy = (bb.minY + bb.maxY) / 2;
      S.drag = {
  ...
  if (at) {
    pushUndo();
    S.selSet = [{ lid: at.lid, idx: at.idx }];
    S.drag = { type: "angle", lid: at.lid, idx: at.idx };
    draw();
    return;
  }
  ...
  if (hit) {
    if (hit.handle) {
      pushUndo();
      S.drag = { type: "handle", side: hit.handle, breakSymmetry: false };
      draw();
      return;
    }
    const hitRef = { lid: hit.lid, idx: hit.idx },
      alreadySel = inSelSet(hitRef);
    if (e.shiftKey) {
      if (alreadySel)
        S.selSet = S.selSet.filter(
          (s) => !(s.lid === hit.lid && s.idx === hit.idx),
        );
      else if (!S.selSet.length || S.selSet[0].lid === hit.lid)
        S.selSet.push(hitRef);
      S.drag = null;
    } else {
  ...
  S.drag = { type: "marquee", startWx: wx, startWy: wy, curWx: wx, curWy: wy };
  draw();
});
...
cv.addEventListener("mousemove", (e) => {
  ...
  document.getElementById("coord").textContent =
    `x:${wx.toFixed(3)}  y:${wy.toFixed(3)}  zoom:${S.cam.z.toFixed(0)}${sn}`;
  if (S.pan) {
    S.cam.x = S.pan.cx - (e.clientX - S.pan.mx) / S.cam.z;
    S.cam.y = S.pan.cy + (e.clientY - S.pan.my) / S.cam.z;
    draw();
    return;
  }
  if (!S.drag) return;
  ...
  if (d.type === "translate") {
    let dx = wx - d.startWx,
      dy = wy - d.startWy;
    if ((S.snap.on || S.snap.pts) && d.origPts.length) {
      const f = d.origPts[0],
        [snx, sny] = snapXY(f.x + dx, f.y + dy, d.origPts);
      dx = snx - f.x;
      dy = sny - f.y;
    }
    for (const op of d.origPts) {
      const p = getPt(op);
      if (!p) continue;
      p.x = r4(op.x + dx);
      p.y = r4(op.y + dy);
    ...
    if (e.altKey) {
      ...
      if (p.tangent) {
        ...
        p.tangentOut = { ...p.tangent };
        p.tangentIn = { ...p.tangent };
        delete p.tangent;
      }
    ...
    const origH = {
      nw: { x: origBBox.minX, y: origBBox.maxY },
      ne: { x: origBBox.maxX, y: origBBox.maxY },
      se: { x: origBBox.maxX, y: origBBox.minY },
      sw: { x: origBBox.minX, y: origBBox.minY },
      n: { x: (origBBox.minX + origBBox.maxX) / 2, y: origBBox.maxY },
      s: { x: (origBBox.minX + origBBox.maxX) / 2, y: origBBox.minY },
      e: { x: origBBox.maxX, y: (origBBox.minY + origBBox.maxY) / 2 },
      w: { x: origBBox.minX, y: (origBBox.minY + origBBox.maxY) / 2 },
    }[handleId];
    if (axis === "x" || axis === "y") {
      ...
      const scX =
        axis === "x"
          ? origH.x === anchor.x
            ? 1
            : (wx - anchor.x) / (origH.x - anchor.x)
          : 1;
      const scY =
        axis === "y"
          ? origH.y === anchor.y
            ? 1
            : (wy - anchor.y) / (origH.y - anchor.y)
          : 1;
      for (const op of origPts) {
        const p = getPt(op);
        if (!p) continue;
        p.x = r4(anchor.x + (op.x - anchor.x) * scX);
        p.y = r4(anchor.y + (op.y - anchor.y) * scY);
      }
    } else if (e.ctrlKey && e.shiftKey) {
      // Ctrl+Shift+drag corner = uniform scale around bbox centroid
      const cent = {
        x: (origBBox.minX + origBBox.maxX) / 2,
        y: (origBBox.minY + origBBox.maxY) / 2,
      };
      const origD = Math.hypot(origH.x - cent.x, origH.y - cent.y);
      const sc =
        origD > 1e-5 ? Math.hypot(wx - cent.x, wy - cent.y) / origD : 1;
      for (const op of origPts) {
        const p = getPt(op);
        if (!p) continue;
        p.x = r4(cent.x + (op.x - cent.x) * sc);
        p.y = r4(cent.y + (op.y - cent.y) * sc);
      }
    } else if (e.shiftKey) {
      // Shift+drag corner = uniform scale from anchor
      const origD = Math.hypot(origH.x - anchor.x, origH.y - anchor.y);
      const sc =
        origD > 1e-5 ? Math.hypot(wx - anchor.x, wy - anchor.y) / origD : 1;
      for (const op of origPts) {
        const p = getPt(op);
        if (!p) continue;
        p.x = r4(anchor.x + (op.x - anchor.x) * sc);
        p.y = r4(anchor.y + (op.y - anchor.y) * sc);
      }
    } else {
      // Plain corner drag = non-uniform scale from anchor
      const scX =
        origH.x === anchor.x ? 1 : (wx - anchor.x) / (origH.x - anchor.x);
      const scY =
        origH.y === anchor.y ? 1 : (wy - anchor.y) / (origH.y - anchor.y);
      for (const op of origPts) {
        const p = getPt(op);
        if (!p) continue;
        p.x = r4(anchor.x + (op.x - anchor.x) * scX);
    ...
    if (e.shiftKey) {
      const s15 = (15 * Math.PI) / 180;
      angle = Math.round(angle / s15) * s15;
    }
    ...
    for (const op of origPts) {
      const p = getPt(op);
      if (!p) continue;
      const rx = op.x - centroid.x,
        ry = op.y - centroid.y;
      p.x = r4(centroid.x + rx * cos - ry * sin);
      p.y = r4(centroid.y + rx * sin + ry * cos);
    }
  } else if (d.type === "angle") {
    const ly = getLayer(d.lid),
      p = ly?.pts[d.idx];
    if (p) {
      const [psx, psy] = w2s(p.x, p.y);
      let ang =
        ((((Math.atan2(sy - psy, sx - psx) * 180) / Math.PI) % 360) + 360) %
        360;
      p.angle = e.shiftKey
        ? (Math.round(ang / 15) * 15) % 360
        : Math.round(ang);
    }
  } else if (d.type === "icon") {
...
cv.addEventListener("mouseup", (e) => {
  ...
  if (S.drag?.type === "marquee") {
    const d = S.drag,
      x0 = Math.min(d.startWx, d.curWx),
      x1 = Math.max(d.startWx, d.curWx),
      y0 = Math.min(d.startWy, d.curWy),
      y1 = Math.max(d.startWy, d.curWy);
    const ly = activeLy();
    if (ly && Math.abs(x1 - x0) > 0.004) {
      const found = [];
      (ly.pts || []).forEach((p, i) => {
        if (p.x >= x0 && p.x <= x1 && p.y >= y0 && p.y <= y1)
          found.push({ lid: ly.id, idx: i });
      });
      if (found.length) S.selSet = e.shiftKey ? [...S.selSet, ...found] : found;
    }
  }
  S.drag = null;
  S.pan = null;
...
cv.addEventListener(
  ...
  (e) => {
    e.preventDefault();
    const { sx, sy } = evCoords(e);
    const [wx0, wy0] = s2w(sx, sy);
    S.cam.z = Math.min(
      3000,
      Math.max(60, S.cam.z * (e.deltaY < 0 ? 1.15 : 0.87)),
    );
    const [wx1, wy1] = s2w(sx, sy);
    S.cam.x += wx0 - wx1;
    S.cam.y += wy0 - wy1;
    draw();
  },
...
document.addEventListener("keydown", (e) => {
  ...
  switch (e.key.toLowerCase()) {
    ...
    case "delete":
    case "backspace":
      if (!S.selSet.length) break;
      pushUndo();
      [...S.selSet]
        .sort((a, b) => b.idx - a.idx)
        .forEach((sl) => {
          const ly = getLayer(sl.lid);
          if (ly) (ly.pts || []).splice(sl.idx, 1);
        });
      S.selSet = [];
      draw();
      break;
...
function fitView() {
  ...
  S.cam.z = Math.max(
    60,
    Math.min((w.clientWidth - 60) / 1.0, (w.clientHeight - 60) / 0.6) * 0.88,
  );
  draw();
}
...
// INIT
loadJSON(JSON.parse(JSON.stringify(PRESETS["standard"])));
